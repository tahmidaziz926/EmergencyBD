...
const tagStyles = {
  tag:  { display: "inline-flex", alignItems: "flex-start", gap: "5px", marginTop: "8px", backgroundColor: "rgba(0,255,136,0.06)", border: "1px solid rgba(0,255,136,0.15)", color: "#00ff88", fontSize: "11px", padding: "5px 10px", borderRadius: "20px", maxWidth: "100%" },
...
const LocationDetail = ({ location }) => {
  ...
  const handleCopy = () => {
    const text = location.lat ? `${location.lat}, ${location.lng}` : location.area;
  ...
  return (
    <div style={locStyles.box}>
      ...
      {location.lat ? (
        <>
          {geocoding ? <p style={locStyles.geocoding}>{"🔄 Fetching address..."}</p>
            : address ? <p style={locStyles.address}>{address}</p> : null}
          <p style={locStyles.coordsSmall}>{location.lat?.toFixed(6)}{", "}{location.lng?.toFixed(6)}</p>
          <button style={locStyles.copyBtn} onClick={handleCopy}>
            {copied ? "✅ Copied!" : "📋 Copy Location Coordinates"}
          </button>
          <a href={`https://www.google.com/maps?q=${location.lat},${location.lng}`} target="_blank" rel="noopener noreferrer" style={locStyles.link}>
        ...
        <>
          ...
          <a href={`https://www.google.com/maps/search/${encodeURIComponent(location.area)}`} target="_blank" rel="noopener noreferrer" style={locStyles.link}>
...
const locStyles = {
  box:        { backgroundColor: "rgba(0,255,136,0.05)", border: "1px solid rgba(0,255,136,0.15)", borderRadius: "10px", padding: "14px", marginBottom: "16px", display: "flex", flexDirection: "column", gap: "8px" },
  title:      { color: "#555555", fontSize: "11px", fontWeight: "600", textTransform: "uppercase", letterSpacing: "0.5px" },
  ...
  copyBtn:    { backgroundColor: "rgba(255,255,255,0.04)", border: "1px solid #2a2a2a", color: "#aaaaaa", padding: "6px 12px", borderRadius: "7px", fontSize: "11px", cursor: "pointer", fontFamily: "inherit", textAlign: "left", width: "fit-content" },
  link:       { color: "#00ff88", fontSize: "12px", fontWeight: "600", textDecoration: "none", display: "inline-block" },
};
      ...
      <div style={lbStyles.container} onClick={e => e.stopPropagation()}>
        <div style={lbStyles.header}>
          ...
          <div style={lbStyles.headerActions}>
            <button style={lbStyles.downloadBtn} onClick={() => downloadImage(imageUrl, filename)}>{"⬇️ Download"}</button>
            <button style={lbStyles.closeBtn} onClick={onClose}>{"✕"}</button>
          </div>
        </div>
        <div style={lbStyles.imageWrap}>
          <img src={imageUrl} alt="report full" style={lbStyles.image} />
        </div>
        <div style={lbStyles.footer}>
          <span style={lbStyles.footerHint}>{"Press ESC or click outside to close"}</span>
        </div>
      </div>
    </div>
  );
...
const lbStyles = {
  overlay:     { position: "fixed", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(0,0,0,0.92)", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center" },
  container:   { backgroundColor: "#111111", border: "1px solid #222222", borderRadius: "16px", overflow: "hidden", maxWidth: "90vw", maxHeight: "90vh", display: "flex", flexDirection: "column", boxShadow: "0 40px 80px rgba(0,0,0,0.8)" },
  header:      { display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 20px", backgroundColor: "#0d0d0d", borderBottom: "1px solid #1e1e1e" },
  headerTitle: { color: "#e0e0e0", fontSize: "14px", fontWeight: "600" },
  headerActions:{ display: "flex", gap: "10px", alignItems: "center" },
  downloadBtn: { backgroundColor: "rgba(0,255,136,0.1)", border: "1px solid rgba(0,255,136,0.25)", color: "#00ff88", padding: "6px 14px", borderRadius: "7px", fontSize: "12px", fontWeight: "600", cursor: "pointer", fontFamily: "inherit" },
  closeBtn:    { backgroundColor: "#1a1a1a", border: "1px solid #2a2a2a", color: "#888888", width: "30px", height: "30px", borderRadius: "50%", cursor: "pointer", fontSize: "13px", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "inherit" },
  imageWrap:   { overflow: "auto", display: "flex", alignItems: "center", justifyContent: "center", padding: "20px", maxHeight: "75vh" },
  image:       { maxWidth: "100%", maxHeight: "70vh", objectFit: "contain", borderRadius: "8px" },
  footer:      { padding: "10px 20px", borderTop: "1px solid #1a1a1a", backgroundColor: "#0d0d0d", textAlign: "center" },
  footerHint:  { color: "#333333", fontSize: "11px" },
};

// ── Quota bar component (compact version for left panel) ──────────────────────
const QuotaBar = ({ quota }) => {
  if (!quota) return null;
  const pct      = Math.min(100, (quota.used / quota.limit) * 100);
  const barColor = quota.exceeded ? "#ff6b6b" : quota.remaining <= 1 ? "#ffaa00" : "#00ff88";
  return (
    <div style={qb.box}>
      <div style={qb.top}>
        <span style={qb.label}>{"📊 Monthly Quota"}</span>
        <span style={{ ...qb.count, color: barColor }}>{quota.used + "/" + quota.limit}</span>
      </div>
      <div style={qb.track}>
        <div style={{ ...qb.fill, width: `${pct}%`, backgroundColor: barColor }} />
      </div>
      <div style={qb.slots}>
        {Array.from({ length: quota.limit }).map((_, i) => (
          <div key={i} style={{
            ...qb.slot,
            backgroundColor: i < quota.used ? barColor : "rgba(255,255,255,0.05)",
          }} />
        ))}
      </div>
      <p style={{ ...qb.note, color: quota.exceeded ? "#ff6b6b" : "#444444" }}>
        {quota.exceeded
          ? "⛔ Limit reached — resets in " + quota.daysUntilReset + " day" + (quota.daysUntilReset !== 1 ? "s" : "")
          : quota.remaining + " report" + (quota.remaining !== 1 ? "s" : "") + " remaining this month"}
      </p>
    </div>
  );
};
...
const qb = {
  box:   { backgroundColor: "#1a1a1a", border: "1px solid #222222", borderRadius: "12px", padding: "14px", display: "flex", flexDirection: "column", gap: "10px" },
  top:   { display: "flex", justifyContent: "space-between", alignItems: "center" },
  label: { color: "#555555", fontSize: "11px", fontWeight: "600", letterSpacing: "0.5px" },
  count: { fontSize: "14px", fontWeight: "800" },
  track: { height: "5px", backgroundColor: "rgba(255,255,255,0.05)", borderRadius: "10px", overflow: "hidden" },
  fill:  { height: "100%", borderRadius: "10px", transition: "width 0.5s ease" },
  slots: { display: "flex", gap: "5px" },
  slot:  { flex: 1, height: "7px", borderRadius: "3px", transition: "background 0.3s ease" },
  note:  { fontSize: "11px", margin: 0 },
};
...
const EmergencyList = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [reportsRes, quotaRes] = await Promise.all([
          axios.get("http://localhost:3001/api/emergency/my-reports", {
            headers: { Authorization: `Bearer ${token}` },
          }),
          axios.get("http://localhost:3001/api/emergency/quota", {
            headers: { Authorization: `Bearer ${token}` },
          }),
        ]);
                ...
                return (
                  ...
                  >
                    <div style={styles.cardTop}>
                      ...
                      <div style={{ ...styles.statusBadge, color: status.color, backgroundColor: status.bg, border: `1px solid ${status.color}30` }}>
                    ...
                    {report.location?.area && <LocationTag location={report.location} />}
                  </div>
                );
              })}
            </div>
        ...
        <div style={styles.rightPanel}>
          {selectedReport ? (
            <div style={{ animation: "slideIn 0.3s ease" }}>
              ...
              <div style={{ ...styles.detailTypeBadge, backgroundColor: `${typeConfig[selectedReport.emergencyType]?.color}10`, border: `1px solid ${typeConfig[selectedReport.emergencyType]?.color}30` }}>
                <span style={styles.detailTypeIcon}>{typeConfig[selectedReport.emergencyType]?.icon}</span>
                <div>
              ...
              <div style={{ ...styles.detailStatusBox, backgroundColor: statusConfig[selectedReport.status]?.bg, border: `1px solid ${statusConfig[selectedReport.status]?.color}30` }}>
              ...
              {selectedReport.imageUrl && (
                <div style={styles.detailSection}>
                  ...
                  <div style={styles.imageActions}>
                    <button className="action-btn" style={styles.imageActionBtn} onClick={() => setLightboxImage(getImageUrl(selectedReport.imageUrl))}>
                      {"🔍 View Full"}
                    </button>
                    <button className="action-btn" style={styles.imageActionBtn} onClick={() => downloadImage(getImageUrl(selectedReport.imageUrl), selectedReport.imageUrl.split("/").pop())}>
                      {"⬇️ Download"}
                    </button>
                  </div>
                </div>
              ...
              <div style={styles.detailMeta}>
                ...
                <div style={styles.metaRow}>
                  ...
                  <span style={styles.metaValue}>
                    {new Date(selectedReport.createdAt).toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit" })}
                  </span>
                </div>
                <div style={styles.metaRow}>
                  <span style={styles.metaLabel}>{"Report ID"}</span>
                  <span style={{ ...styles.metaValue, fontSize: "10px", color: "#444444" }}>{selectedReport._id}</span>
                </div>
              </div>
            </div>
          ) : (
...
const styles = {
  ...
  leftPanel:    { width: "260px", minWidth: "260px", backgroundColor: "#0d0d0d", borderRight: "1px solid #1e1e1e", padding: "28px 20px", display: "flex", flexDirection: "column", gap: "20px", overflowY: "auto" },
  ...
  newBtn:       { backgroundColor: "#00ff88", color: "#0a0a0a", border: "none", padding: "8px 14px", borderRadius: "8px", fontSize: "13px", fontWeight: "700", cursor: "pointer", flexShrink: 0 },
  ...
  filterBtn:    { display: "flex", justifyContent: "space-between", alignItems: "center", padding: "9px 12px", borderRadius: "8px", border: "1px solid transparent", backgroundColor: "transparent", color: "#666666", fontSize: "13px", cursor: "pointer", textAlign: "left", width: "100%" },
  filterActive: { backgroundColor: "rgba(0,255,136,0.08)", border: "1px solid rgba(0,255,136,0.2)", color: "#00ff88" },
  filterCount:  { backgroundColor: "#222222", color: "#555555", fontSize: "11px", fontWeight: "600", padding: "2px 8px", borderRadius: "10px" },
  statsBox:     { display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px", backgroundColor: "#1a1a1a", border: "1px solid #222222", borderRadius: "12px", padding: "16px" },
  ...
  emptyState:   { display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "100%", gap: "16px", padding: "60px 20px", textAlign: "center" },
  ...
  emptyBtn:     { backgroundColor: "#00ff88", color: "#0a0a0a", border: "none", padding: "12px 24px", borderRadius: "10px", fontSize: "14px", fontWeight: "700", cursor: "pointer", marginTop: "8px" },
  reportList:   { display: "flex", flexDirection: "column" },
  reportCard:   { padding: "20px 24px", borderBottom: "1px solid #1a1a1a", cursor: "pointer", animation: "fadeUp 0.4s ease both", border: "1px solid transparent", borderBottomColor: "#1a1a1a" },
  cardTop:      { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "10px" },
  cardLeft:     { display: "flex", alignItems: "center", gap: "12px" },
  typeIconBox:  { width: "40px", height: "40px", borderRadius: "10px", display: "flex", alignItems: "center", justifyContent: "center" },
  typeIcon:     { fontSize: "18px" },
  cardType:     { fontSize: "13px", fontWeight: "700" },
  cardDate:     { color: "#444444", fontSize: "11px", marginTop: "2px" },
  statusBadge:  { fontSize: "11px", fontWeight: "600", padding: "4px 10px", borderRadius: "20px", letterSpacing: "0.3px" },
  ...
  detailTypeBadge: { display: "flex", alignItems: "center", gap: "14px", borderRadius: "12px", padding: "16px", marginBottom: "14px" },
  ...
  detailStatusBox: { display: "flex", alignItems: "center", gap: "14px", borderRadius: "12px", padding: "14px", marginBottom: "16px" },
  detailStatusDesc:{ color: "#444444", fontSize: "11px", marginTop: "2px" },
  detailSection:   { marginBottom: "20px" },
  detailSectionTitle:{ color: "#555555", fontSize: "11px", fontWeight: "600", marginBottom: "8px", textTransform: "uppercase", letterSpacing: "0.5px" },
  detailDescription: { color: "#cccccc", fontSize: "13px", lineHeight: "1.7", backgroundColor: "#1a1a1a", border: "1px solid #222222", borderRadius: "10px", padding: "14px" },
  imageThumbWrap:  { position: "relative", cursor: "pointer", borderRadius: "10px", overflow: "hidden", marginBottom: "8px" },
  detailImage:     { width: "100%", borderRadius: "10px", objectFit: "cover", maxHeight: "180px", display: "block", transition: "opacity 0.3s ease" },
  imageFallback:   { display: "none", backgroundColor: "#1a1a1a", border: "1px solid #222222", borderRadius: "10px", padding: "20px", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: "8px" },
  imageActions:    { display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px" },
  imageActionBtn:  { backgroundColor: "rgba(0,255,136,0.08)", border: "1px solid rgba(0,255,136,0.2)", color: "#00ff88", padding: "8px", borderRadius: "8px", fontSize: "12px", fontWeight: "600", cursor: "pointer", fontFamily: "inherit", textAlign: "center", transition: "all 0.3s ease" },
  detailMeta:      { backgroundColor: "#1a1a1a", border: "1px solid #222222", borderRadius: "12px", padding: "16px", display: "flex", flexDirection: "column", gap: "12px" },
